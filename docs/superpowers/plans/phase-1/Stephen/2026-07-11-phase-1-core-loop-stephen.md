...
**Post-implementation review note (2026-07-17):** Approved after one fix round. `themeId` was originally derived as `question.themeIds[0]` — an arbitrary index — instead of the theme owning the LO actually served; wrong for any question tagged across multiple themes, and it corrupted `AttemptRecord.themeId`, the Review Book entry, and the `themeCoverage()` check backing `recommendation: 'advance-theme'`. Fixed via a `losCol()` lookup on the served `loId`; a regression test now tags a question across two themes to pin the fix. `recommendation` also went from zero test coverage to real assertions on both `'advance-lo'` and `'advance-theme'` (theme supersedes LO when one attempt causes both — a documented, deliberate reading of the spec). One deferred ⚠️, pre-existing and cross-task: `editQuestion` (Saurav's Task 4, already on `main`) doesn't reset a question's `state` to `draft` on a post-approval edit, so `submitAttempt`'s `state === 'approved'` gate doesn't catch a student holding a stale `questionVersionId` from before the edit — worth raising at the Task 16 exit review. Full detail in the gitignored `.superpowers/sdd/progress.md` / `task-11-report.md`.
...
- [x] **Step 1: Write the failing tests** — `tests/unit/review-book.service.test.ts` per the core document, Task 12 Step 1 (bookmark toggle on an auto-collected entry keeps `sources:['auto', ...]` behavior correctly scoped; bookmark on a never-missed question creates a fresh `sources:['bookmark']` entry; `removeEntry` never calls `attemptsCol`; `listReviewBook` groups by theme with counts and honours `date` sort; `sessionEndSummary`'s `missedQuestions` ids equal the review-book additions in the window — not a divergently-computed list, ST-R06).
- [x] **Step 2: Run tests to verify they fail** — `npx jest tests/unit/review-book.service.test.ts` → FAIL.
- [x] **Step 3: Implement** service + routes (`GET /api/courses/:courseId/review-book?sort=`, `POST/DELETE /api/questions/:questionId/bookmark`, `DELETE /api/review-book/:entryId`, the two summary endpoints); add the `sessionSummaries` collection accessor/index; mount in `app.ts`.
- [x] **Step 4: Run tests** — `npx jest tests/unit/review-book.service.test.ts && npm run typecheck` → PASS.
- [x] **Step 5: Commit** — `git commit -m "feat: review book browsing, bookmarking, and session summaries (ST-R02..R07, ST-P10/P11)"`

**Post-implementation review note (2026-07-17):** Approved after one fix round. Shipped a new `PUT /api/courses/:courseId/deferred-summary` route without a `docs/api-contract.md` entry, violating the "new endpoints match the contract" global constraint (plan-mandated Important finding). Fixed with a docs-only commit (`1579a6b`) adding the contract entry — request/response shapes cross-checked against the actual route/service code — and tightening the `GET .../session-summary` entry to its real `{ deferred?, welcome }` shape. Deferred Minors (not blocking): `attempts.service.ts`'s old `getSessionSummary`/`SessionSummary` placeholder is now dead code with a stale docstring (worth a follow-up cleanup); no `review-book.routes.test.ts` (route-layer test) exists, only the service test — out of scope per the brief's file list; `listReviewBook` silently drops entries whose `themeId` no longer resolves to a theme document (no error surfaced).
...
- [x] **Step 1: Extend the router with param matching + unit-test it** — `tests/unit/client-router.test.ts` (or a pure-function test on the extracted `matchRoute(pattern, path)` helper if no jsdom test env is configured). **Check with Saurav first** whether his Task 15 has already landed this — reuse rather than duplicate.
- [x] **Step 2: Build the views one route at a time**, verifying each in the browser against seeded data (seed via the Task 4/5 service snippet in Task 10 above, or curl). Keep each view file under ~200 lines; shared bits (option buttons, status badge) go in `client/src/ui.ts` (coordinate with Saurav's Task 15, which also touches `ui.ts`).
- [x] **Step 3: Typecheck + lint after each view** — `npm run typecheck && npm run lint` → PASS.
- [x] **Step 4: Playwright happy-path spec** `tests/e2e/practice-loop.spec.ts` — student joins course (pre-seeded via API in `beforeAll` using an instructor session), practices one question, sees feedback, misses one, finds it in the Review Book. Run: `npm run test:e2e -- tests/e2e/practice-loop.spec.ts` → PASS.
- [x] **Step 5: Commit** — `git commit -m "feat: student practice, review book, and session summary views (ST-P01..P11, ST-R05)"`

**Post-implementation review note (2026-07-17):** Approved after one fix round. Router extended with a pure, DOM-free, TDD'd `matchRoute` helper (`client/src/route-match.ts`, 9 unit tests); five student views + `practice-session.ts` built and wired; a live Playwright e2e run (real MongoDB/SAML-IdP/FakeAcademicAPI in the sandbox) actually passed, not just written blind. Review found three Important, plan-mandated gaps, all fixed in one round: (1) no "End session" action from the practice view — added a header link to `/course/:id/summary?since=<session start>`, reusing the existing `deferSessionSummary` call; (2) no "practice this LO more" link per transcript entry (ST-P08) — added `loId` to `TranscriptEntry`, verified no closure-capture bug across retry-in-place/LO-advance; (3) submit-attempt failure had no retry path, permanently soft-locking the card on a transient network error — added `onRetry` matching the pattern at every other error site, verified race-free with no double-submit. Deferred Minors (not blocking): `session-summary.ts` shows raw `loId` strings instead of LO names (contract has no `loName` field, needs a cross-task follow-up); `docs/api-contract.md`'s `/practice/next` response shape is stale (documents a nested shape; real shape is flat) — pre-existing drift from Task 11; the e2e spec doesn't assert on the three fix-round links specifically. Full detail in the gitignored `.superpowers/sdd/progress.md` / `task-14-report.md`.
