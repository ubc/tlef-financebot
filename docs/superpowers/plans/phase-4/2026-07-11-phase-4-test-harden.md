...
- [ ] **Step 3: Run the full e2e suite three times consecutively** — `for i in 1 2 3; do npm run test:e2e || break; done` → 3× PASS (flake check; fix any flaky waits with proper `expect(...).toBeVisible()` polling, never `waitForTimeout`).
      **Blocked on a decision, not on work (updated 2026-08-01, `main` @ `4761b74`).** Three consecutive full runs give an identical **23 passed / 2 failed / 1 skipped** — deterministic, not flaky.
      The two failures are `app.spec.ts:10` and `walking-skeleton.spec.ts:14`. Both navigate to `/` and assert the heading reads "My Courses". `client/src/main.ts:345` makes the root fallback role-dependent (`isAdmin` → `#/admin/accounts`), and `isAdmin` is written at first login from `ADMIN_CWL_ALLOWLIST` and **never revoked** when that list later changes — so the assertion holds or fails on ambient dev-database state. Both still pass their *navigation* assertions; it is specifically the heading that differs.
      Deliberately unpatched: `walking-skeleton` is named "a logged-in user sees a **role-appropriate** home", so hardcoding one role's heading is arguably the real defect there and the fix is per-role assertions — a different change from the navigate-explicitly one applied to `instructor-pipeline.spec.ts` in PR #56. Both are also Phase 0 specs rather than clearly Saurav-owned.
      Options, the recommendation, and the question that decides it (*was `ADMIN_CWL_ALLOWLIST=12345678` deliberate or a leftover?*) are in [`Saurav/STATUS.md`](Saurav/STATUS.md).
      *Superseded:* the earlier blocker recorded here was `flag-loop.spec.ts`'s cross-course notification residue. That is **fixed** in PR #56, mutation-verified so it fails rather than passing on stale data.
