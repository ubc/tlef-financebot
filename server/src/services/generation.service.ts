...
export function GENERATOR_PROMPT(params: {
  ...
  return [
    ...
    '  2. Option values must differ for EVERY value in range — distractors must never',
    '     coincide with the correct answer or with each other at any draw. Beware',
    '     formulas that collide at a range endpoint (an "n periods" distractor equals the',
    '     correct answer when n can draw 1).',
    '     REVERSED SUBTRACTION is the most common way to break this rule. If the correct',
    '     value is "A - B", do NOT use "B - A" as a distractor unless the ranges of A and',
    '     B cannot overlap: wherever A equals B both are 0 and the two options are',
    '     identical. Either give A and B non-overlapping ranges, or use a different',
    '     mistake (a wrong operand, a wrong rate, a dropped term).',
    '',
